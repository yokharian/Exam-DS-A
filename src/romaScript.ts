...
export function convertToRoman(num: number): string {
  // 0 < num <= 10000
  const magnitudes = ((val: number): number[] =>
    val
      .toString()
      .split('')
      .map(v => parseInt(v, 10))
  ...
  const findByValue = (
    value: any,
    objeto = romanianNotation,
  ): (string | number)[] => {
    return Object.entries(objeto).find(
      entrie => entrie[1] === Object.values(objeto).find(val => val === value),
    );
  };
  ...
  const getZeros = (v: number) => Math.pow(10, Math.trunc(Math.log10(v)));

  const isRestable = (
    difWithMinus: (string | number)[],
    difWithPlus: (string | number)[],
  ) => {
    const difWithMinusValue = difWithMinus[1] as number;
  ...
  return magnitudes
    .reduce((output: Array<string>, magnitud: number) => {
      let _COUNTER = 0;
      let _ACCOUTPUT = '';
      ...
      while (_COUNTER !== magnitud) {
        ...
        if (isRestable(difWithMinus, difWithPlus)) {
          _ACCOUTPUT +=
            (findByValue(difWithMinus[1])[0] as string) + difWithMinus[0];
          _COUNTER += magnitud;
        } else {
