...
function convertToRoman(num) {
    ...
    const findByValue = (value, objeto = romanianNotation) => {
        return Object.entries(objeto).find(entrie => entrie[1] === Object.values(objeto).find(val => val === value));
    };
    const miniumDif = (suma = true, magnitud, _COUNTER = 0) => {
        var _a;
        const archivo = Object.entries(romanianNotation).filter(v => suma ? v[1] <= magnitud : v[1] >= magnitud);
        return ((_a = archivo
            .map(actual => {
            const key = actual[0];
            ...
            .shift()) !== null && _a !== void 0 ? _a : ['M', 1000]);
    };
    ...
    return magnitudes
        ...
        while (_COUNTER !== magnitud) {
            ...
            if (isRestable(difWithMinus, difWithPlus)) {
                _ACCOUTPUT +=
                    findByValue(difWithMinus[1])[0] + difWithMinus[0];
                _COUNTER += magnitud;
            }
            else {
                _ACCOUTPUT += difWithPlus[0];
                _COUNTER += Object.entries(romanianNotation).find(v => v[0] === difWithPlus[0])[1];
            }
        }
...
//# sourceMappingURL=romaScript.js.map